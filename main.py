                            ...
                            if real_url:
                                ...
                                if only_flv_record:
                                    ...
                                    if split_video_by_time:
                                        ...
                                        try:
                                            save_file_path = f"{full_path}/{anchor_name}_{title_in_name}{now}.ts"
                                            command = [
                                                "-c:v", "copy",
                                                "-c:a", "copy",
                                                "-map", "0",
...
print(f"版本号: {version}")
print("GitHub: https://github.com/sjwayrhz/DouyinLiveRecorder")
...
while True:
    ...
    try:
        ...
        # 检测 running_list 中已从文件彻底删除的 URL（既不在正常行，也不在注释行）
        # 将其加入 url_comments，触发对应录制线程自动退出
        all_urls_in_file = set(url_comments) | {t[1] for t in url_tuples_list}
        for removed_url in running_list:
